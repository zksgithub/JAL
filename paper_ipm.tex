% ============================================================
%  IPM submission — hybrid LLM-KG extraction for biographical text
% ============================================================
...
\title{Beyond Passive Custodianship: A Hybrid LLM--Knowledge Graph\\\\
       Pipeline for Computational Extraction and Quality Assessment\\\\
       of Unstructured Biographical Text}

\begin{abstract}
Unstructured biographical text—whether oral histories, chronological biographies, or archival finding aids—constitutes a vast but computationally under-exploited information resource. Traditional approaches treat such text as human-readable narrative rather than machine-queryable data, leaving latent relational information (mentorship, collaboration, institutional affiliations) inaccessible to structured retrieval. This paper presents a three-tier hybrid extraction architecture that combines deterministic regular-expression parsing, large language model (LLM)-based deep semantic extraction, and multi-relational knowledge graph construction to transform unstructured biographical text into structured, queryable linked data. Applied to a corpus of 26 Chinese scientist chronological biographies (nianbiao, 1898--1935, 2,582 discrete life events), the pipeline achieves 97.5\% education entity precision and 88.7\% award precision, recovering 1,765 structured facts across nine categories—including 74 mentors, 268 collaborators, and 523 life events that are entirely absent from Wikidata (measured against a full 142~GB Wikidata JSON dump; Wikidata Enrichment Ratio = 36.8$\times$). We propose five computable quality indicators—Uncertainty Density (UD, range 1.12--5.96), Memory Bias Index (MBI, $\chi^2 = 842.2$, $p < 10^{-172}$), Cross-Reference Connectivity (CRC = 0.0\%), Narrative Arc Type (NAT, 57.7\% TRIUMPH), and Wikidata Enrichment Ratio (WER = 36.8$\times$)—that together form a closed-loop assessment framework for any unstructured biographical corpus. An ablation study isolates the marginal contribution of the LLM component, and a systematic Wikidata comparison quantifies the complementarity gap between domain-specific text extraction and general-purpose knowledge graphs. The framework replaces the traditional model of passive text archiving with computationally-driven, iterative extraction and quality assessment, turning unstructured biographical collections from static documents into active, queryable information assets.
\end{abstract}

\begin{keyword}
knowledge graph \enspace$\bullet$\enspace large language models \enspace$\bullet$\enspace information extraction \enspace$\bullet$\enspace unstructured text \enspace$\bullet$\enspace biographical data \enspace$\bullet$\enspace Wikidata \enspace$\bullet$\enspace quality metrics
\end{keyword}
...
Unstructured biographical text represents an enormous, largely untapped information resource. Oral histories, chronological biographies, archival finding aids, and institutional records collectively encode millions of latent relational facts—who mentored whom, who collaborated with whom on which project, how scientific careers traversed institutions and historical periods. Yet these connections remain locked in narrative prose, inaccessible to structured query, computational analysis, or integration with external knowledge bases. The challenge is fundamentally one of information extraction: transforming narrative text into machine-readable, relationally structured data without sacrificing the contextual richness that makes biographical text valuable in the first place.

Recent advances in large language models (LLMs) have dramatically expanded the scope of feasible information extraction. Unlike traditional named entity recognition (NER) pipelines that require curated entity dictionaries and struggle with implicit relationships, LLMs can perform zero-shot semantic extraction—identifying entities, classifying relationship types, and disambiguating context (e.g., distinguishing \textit{attended} from \textit{declined} educational opportunities) from unstructured text alone \cite{pan2023unifying}. When combined with knowledge graph (KG) construction, LLM extraction offers a pathway to transform isolated biographical narratives into connected, queryable linked data, bridging the gap between human-readable archives and machine-actionable knowledge bases such as Wikidata.

This study is motivated by three unresolved challenges at this intersection. First, extraction quality from unstructured biographical text lacks standardized, reproducible benchmarks. While LLM extraction has been evaluated on news corpora and Wikipedia-scale entity datasets, its performance on domain-specific biographical text—with its genre conventions, implicit relationships, and narrative structure—remains unmeasured. Second, the complementarity between LLM-extracted domain data and general-purpose knowledge graphs has not been systematically quantified: Wikidata and similar platforms provide broad identity resolution but their coverage depth for domain-specific relational facts is unknown. Third, the iterative enrichment loop—extract, compare against public KGs, enrich, re-assess—lacks an evaluative framework that can identify what remains uniquely scarce after maximum computational and public-data enrichment.

To address these challenges, we present a three-tier hybrid extraction architecture combining deterministic regular-expression parsing (Tier~1), LLM-based deep semantic extraction (Tier~2), and multi-relational knowledge graph construction (Tier~3), applied to a corpus of 26 Chinese scientist chronological biographies (nianbiao, 1898–1935, 2,582 discrete life events). The corpus domain—modern Chinese scientific biography—is deliberately chosen for its unstructured text characteristics: genre-specific conventions, implicit relational data, and documented historical silences that stress-test extraction pipelines. We evaluate extraction quality through systematic precision measurement (97.5\% education precision, 88.7\% award precision), compare LLM-extracted facts against a full 142~GB Wikidata JSON dump, and propose five computable quality indicators (MI1--MI5) that operationalize uncertainty, memory bias, cross-reference connectivity, narrative structure, and Wikidata complementarity as quantitative metrics.

This study addresses three research questions:

\begin{enumerate}[label=RQ\arabic*:,leftmargin=*]
    \item Can the content quality of unstructured biographical text be computationally quantified through a unified extraction and indicator framework?
    \item What gaps exist between LLM-extracted domain-specific data and public knowledge graphs such as Wikidata, and how do these sources complement each other?
    \item Does targeted enrichment with public data improve extraction quality, and what residual information—uniquely scarce content that no external source can fill—remains after maximum enrichment?
\end{enumerate}

This study makes three contributions. Collectively, they advance information extraction from unstructured text by replacing the traditional model of passive archival with a computationally-driven, iterative extraction and quality assessment framework.

\textbf{C1: A validated hybrid extraction architecture.} We present and evaluate a three-tier pipeline (regex baseline + LLM deep extraction + KG construction) with systematic precision measurement across nine entity categories. An ablation study isolates the marginal contribution of the LLM component over the regex-only baseline.

\textbf{C2: Empirical quantification of the domain-KG complementarity gap.} Through comparison with a full 142~GB Wikidata dump, we provide the first systematic measurement of complementarity between LLM-extracted domain-specific biographical data and a general-purpose knowledge graph, revealing zero relational coverage (mentorship, collaboration, life events) and a 36.8$\times$ Wikidata Enrichment Ratio.

\textbf{C3: A five-indicator quality assessment framework (MI1--MI5).} We propose Uncertainty Density (UD), Memory Bias Index (MBI), Cross-Reference Connectivity (CRC), Narrative Arc Type (NAT), and Wikidata Enrichment Ratio (WER)—five computable metrics that form a closed-loop assessment framework (assess $\rightarrow$ enrich $\rightarrow$ re-assess $\rightarrow$ follow up) applicable to any unstructured biographical corpus. The framework identifies three categories of irreducibly scarce content that persist after maximum enrichment: personal reflections never formally published, tacit experimental knowledge transmitted only orally, and informal mentor networks documented exclusively in personal chronologies.

% ============================================================
\section{Related Work}
% ============================================================

\subsection{Information Extraction from Unstructured Text}

The transformation of unstructured text into structured knowledge has been a central concern of information processing research for decades. Traditional approaches relied on pipeline architectures—tokenization, part-of-speech tagging, named entity recognition (NER), relation extraction—each requiring domain-specific dictionaries and manually crafted rules. For biographical text specifically, Gottschalk and Demidova \cite{gottschalk2019eventkg} developed EventKG, an event-centric knowledge graph that generates biographical timelines from structured sources, demonstrating the conceptual alignment between event-based data models and biographical narratives. Liu and Wang \cite{liu2017matrix} applied graph-based relationship inference to the China Biographical Database (CBDB), showing how computational methods can extract kinship and social ties from historical Chinese biographical records. Blanke and Riechert \cite{blanke2020rdf} demonstrated RDF knowledge graph construction for early modern scholars, establishing a prosopographical methodology that treats individuals as nodes in a relational network rather than as isolated biographical entries.

The emergence of large language models has fundamentally altered the extraction landscape. Pan et al.\ \cite{pan2023unifying} surveyed the unification of LLMs and KGs, identifying entity extraction and relationship inference as especially promising applications where LLMs can replace brittle, dictionary-dependent pipelines. De Paiva and Rademaker \cite{depaiva2024brazilian} demonstrated a domain-specific history KG construction pipeline using LLM-assisted extraction, showing that semi-automated methods can transform unstructured historical texts into queryable linked data. Carriero et al.\ \cite{carriero2024human} conducted a systematic human evaluation of procedural KG extraction with LLMs, finding that structured prompting with entity-type schemas significantly improves extraction precision—a design principle informing our structured nine-category extraction prompt (Section~\ref{sec:method}). Lu et al.\ \cite{lu2025karma} proposed KARMA, a multi-agent LLM framework for automated KG enrichment that addresses scalability challenges in manual curation, though their evaluation focused on general-domain KGs rather than domain-specific biographical corpora.

Despite these advances, systematic precision evaluation of LLM extraction on unstructured biographical text—with its genre conventions, implicit relationships, and narrative ambiguity—remains sparse. Most evaluations use news corpora or Wikipedia-derived datasets, where facts are explicitly stated and entity boundaries are well-defined. Our work contributes precision measurements across nine entity categories on a domain-specific biographical corpus, including an ablation study that isolates the LLM component's marginal contribution (Section~\ref{sec:precision}).

\subsection{Biographical Text and Linked Data Models}

Biographical text carries structural properties that distinguish it from general-domain prose. The Chinese nianbiao (chronological biography), originating in Song dynasty historiography, presents life events in strict temporal order with precise dates and institutional contexts \cite{twitchett1962chinese,wilkinson2018chinese}—a format that maps naturally to event-centric knowledge graph models. The CIDOC-CRM ontology provides a formal conceptual framework for modeling biographical events, their temporal scopes, and institutional contexts \cite{doerr2003cidoc}, while initiatives such as SNAC (Social Networks and Archival Context) and VIAF have demonstrated the practical feasibility of person-centric linked data at scale \cite{larson2015connecting}.

The oral history tradition provides theoretical grounding for treating biographical text as constructed narrative rather than objective record. Portelli \cite{portelli1991death} established that oral testimonies encode memory, identity, and interview context in their narrative structure; Thompson \cite{thompson2000voice} argued that narrative form itself is a source of evidence. These insights directly inform our treatment of nianbiao as curated retrospective accounts subject to genre conventions, memory bias, and narrative isolation—characteristics that our indicators (MI1--MI3) are designed to detect and quantify.

Sherratt \cite{sherratt2020biographical} characterized the ``biographical turn'' in digital humanities, arguing that computational methods enable the analysis of life narratives at corpus scale—a shift from reading individual biographies to measuring systematic patterns across hundreds or thousands of life stories. Trainin et al.\ \cite{trainin2026testimony} recently proposed a scalable framework for comparing oral history archives through computational narrative analysis, demonstrating that structural patterns—including narrative arc distributions—constitute a new dimension of metadata. Our narrative arc classification (NAT, MI4) extends this approach by integrating it into a broader multi-indicator framework.

\subsection{Knowledge Graph Enrichment and Wikidata}

General-purpose knowledge graphs, particularly Wikidata, have achieved remarkable breadth in identity resolution, covering millions of entities with basic demographic properties (birth/death, nationality, occupation) \cite{allisoncassin2018wikidata}. However, their depth for domain-specific relational data remains poorly characterized. While SPARQL-based Wikidata coverage studies exist for specific domains, no systematic measurement of the complementarity gap between domain-specific text extraction and Wikidata—quantified as a per-collection enrichment ratio—has, to our knowledge, been published.

This gap is significant for information extraction research. If LLM-extracted domain data largely duplicates existing Wikidata statements, the extraction pipeline adds limited value; if whole relationship categories are absent from Wikidata, domain extraction becomes an essential complement to general-purpose KGs. Our Wikidata Enrichment Ratio (WER, MI5) provides this quantification through a full 142~GB dump-level comparison (Section~\ref{sec:wikidata}), revealing that mentorship, collaboration, and biographical events—three relationship types critical to scientific biography—are entirely absent from Wikidata for our cohort, while LLM extraction recovers 1,765 structured facts (36.8$\times$ enrichment). This finding positions domain-specific extraction as an architectural complement to general-purpose KGs, not a competitor.
...
\textbf{Tier 3: Cross-Validation and Knowledge Graph Construction.} LLM-extracted entities and relationships were cross-validated against the regex baseline for factual consistency (e.g., birth/death year agreement, institution name normalization). Validated entities were then modeled as a multi-relational knowledge graph in NetworkX, with nodes representing scientists, institutions, and persons, and typed edges encoding educational affiliation, career tenure, mentorship, collaboration, and family relationships. Algorithmically, the pipeline proceeds as: (1) extract baseline facts via regex from each text, (2) submit each text to the LLM with the structured 9-category prompt and parse the JSON output, (3) cross-validate LLM output against regex baseline for consistency, (4) normalize institution names to canonical form, (5) construct nodes for scientists, persons, and institutions, and (6) create typed edges from the extracted relationship tuples.

\subsection{Ablation Study Design}

To isolate the marginal contribution of the LLM component, we designed a two-condition ablation. \textbf{Condition A (Regex-only):} the Tier~1 deterministic regex parser extracts birth/death years, page markers, and year-referenced events—representing the upper bound of what traditional rule-based metadata extraction can achieve. \textbf{Condition B (Full pipeline):} all three tiers are applied, producing the complete extraction of nine entity categories with relationship types. The delta between Conditions~A and B quantifies the LLM component's contribution in three dimensions: (1) entity count (raw number of extracted facts), (2) entity type coverage (number of categories where at least one fact is found), and (3) relationship type coverage (number of distinct edge types in the resulting KG). Table~\ref{tab:ner_comparison} in Section~\ref{sec:results} reports the ablation results.
...
The findings of this study converge on four interconnected arguments that, taken together, describe a complete information processing pipeline for transforming unstructured biographical text into computable, interoperable, and globally connected structured data. These arguments map to the three research questions: RQ1 (extraction quality quantification) to the narrative-to-metric transformation and MI1--MI5 indicators; RQ2 (complementing public knowledge graphs) to the Wikidata gap analysis; and RQ3 (post-enrichment assessment) to the alignment scenarios and closed-loop quality assessment framework.
...
The narrative arc is one of five computable indicators---four from the oral history experiments (Section~\ref{sec:oral}) plus one from the Wikidata comparison (Section~\ref{sec:wikidata})---that together constitute a dashboard for data-informed collection assessment. We propose the following five metrics, designated MI1 through MI5. Table~\ref{tab:indicators} provides a summary.

\begin{table}[ht]
\centering
\caption{Five computable quality indicators for unstructured biographical corpora.}
\label{tab:indicators}
\begin{tabular}{lcll}
\toprule
Indicator & Formula & Provenance & Curatorial Action \\
\midrule
MI1: UD & $\text{UD} = \frac{N_{\text{markers}}}{N_{\text{chars}} / 1000}$ & This study & Flag for fact-checking \\
MI2: MBI & $z_d = \frac{O_d - E}{\sqrt{E}}$ & $\chi^2$ framework \cite{rubin1986autobiographical}; operationalization ours & Acquire supplementary records \\
MI3: CRC & $\text{CRC} = \frac{C_{\text{confirmed}}}{C_{\text{total}}} \times 100\%$ & Adapted from SNAC \cite{larson2015connecting} & Cross-reference project targeting \\
MI4: NAT & $\text{NA} = \underset{a \in \mathcal{A}}{\arg\max}\, P(a \mid \text{events})$  & Archetypes from Reagan et al.\ \cite{reagan2016emotional} & Faceted browsing, gap analysis \\
MI5: WER & $\text{WER} = \frac{F_{\text{LLM}}}{F_{\text{WD}}}$ & This study & Wikidata publication prioritization \\
\bottomrule
\end{tabular}
\end{table}

\textbf{MI1: Uncertainty Density (UD).} UD quantifies the frequency of linguistic uncertainty markers in oral testimony, capturing the degree to which a narrative relies on hedging, approximation, and inferential language rather than declarative factual statements. It is computed as:

\begin{equation}
\text{UD} = \frac{N_{\text{markers}}}{N_{\text{chars}} / 1000}
\end{equation}

where $N_{\text{markers}}$ is the total count of markers across six categories---approximation (e.g., \textit{dayue} ``approximately''), hedging (\textit{keneng} ``possibly''), memory uncertainty (\textit{jushuo} ``reportedly''), vagueness (\textit{buxiang} ``unclear''), inference (\textit{ying} ``should be''), and contradiction (\textit{lingshuo} ``another account says'')---identified via regex patterns described in Section~\ref{sec:oral}, and $N_{\text{chars}}$ is the total character count of the source text. Across all 26 chronologies, 1,037 markers were identified: inference markers dominated (751, 72.4\%), followed by approximation (234, 22.6\%), hedging (35, 3.4\%), memory uncertainty (14, 1.3\%), and vagueness (3, 0.3\%); no contradiction markers were found. Our corpus yields UD values ranging from 1.12 (Pan Jiazheng) to 5.96 (Tang Mingshu) markers per 1,000 characters ($5.3\times$ range). UD is proposed here as a novel curatorial metric; while uncertainty detection in text has precedent in sentiment analysis and factuality evaluation \cite{griffin2014text}, its operationalization as a collection-level quality indicator is, to our knowledge, original. \textbf{Curatorial action:} high-UD collections are flagged for fact-checking or supplementary source verification. A \texttt{certainty} field (CERTAIN / PROBABLE / UNCERTAIN) can be auto-populated from LLM extraction with marker context.

\textbf{MI2: Memory Bias Index (MBI).} MBI identifies decades in which event density deviates systematically from expectation, revealing under-documented periods that constitute ``historical silences.'' It is computed as the standardized residual ($z$-score) in a $\chi^2$ goodness-of-fit test comparing observed event counts per decade ($O_d$) against the uniform-distribution expectation ($E = \sum O_d / D$, where $D$ is the number of decades):

\begin{equation}
z_d = \frac{O_d - E}{\sqrt{E}}
\end{equation}

with overall significance assessed via $\chi^2 = \sum_d z_d^2$, $df = D - 1$ ($D = 13$ decades, 1930s--2000s). Three decades were significantly over-represented---1930s ($z = +8.2$), 1940s ($z = +20.5$), 1950s ($z = +10.0$)---collectively accounting for 58.1\% of all dated events and corresponding to the scientists' early careers and the founding of the People's Republic. Critically, the 1960s ($z = -2.9$) and 1970s ($z = -2.7$) show significant under-representation, with only 10.3\% of events despite covering 20 years, coincident with the Cultural Revolution (1966--1976). The $\chi^2$ framework is standard inferential statistics; the reminiscence bump concept originates in autobiographical memory research \cite{rubin1986autobiographical}. Our contribution is the operationalization of per-decade $z_d$ as a named, actionable curation metric (MBI) that maps statistical significance to specific archival interventions. \textbf{Curatorial action:} flag temporal gaps in metadata; prioritize acquisition of contemporaneous institutional records to fill documented silences.

\textbf{MI3: Cross-Reference Connectivity (CRC).} CRC measures the degree to which fact claims in one oral history collection are reciprocally confirmed by at least one other collection in the same corpus. It is computed as:

\begin{equation}
\text{CRC} = \frac{C_{\text{confirmed}}}{C_{\text{total}}} \times 100\%
\end{equation}

where $C_{\text{total}}$ is the total number of verifiable fact claims spanning three dimensions---career organizations (353), education institutions (210), and named collaborators (268), totaling 831 verifiable claims across 26 chronologies---and $C_{\text{confirmed}}$ is the subset confirmed by $\geq 1$ other collection. Our corpus: CRC = 0.0\%, i.e., complete narrative isolation---only one pair of scientists (Feng Duan and Wang Dezi) shared a career organization (Nanjing University), and zero collaborator cross-references were found, confirming that these chronologies were compiled independently without cross-referencing. CRC is conceptually inspired by the Social Networks and Archival Context (SNAC) project's vision of interconnected archival description \cite{larson2015connecting}, adapted here into a quantitative collection-level metric. \textbf{Curatorial action:} low-CRC collections identified for cross-referencing projects; high-CRC clusters indicate well-triangulated factual cores suitable as linked data anchors.

\textbf{MI4: Narrative Arc Type (NAT).} NAT classifies each scientist's life trajectory into one of four archetypes, operationalizing the computational narratology framework of Reagan et al.\ \cite{reagan2016emotional}, who demonstrated that emotional arcs follow six quantifiable shapes. For the specific genre constraints of Chinese nianbiao---honorific commemoration emphasizing institutional milestones---we adapt these to four archetypes: TRIUMPH (steady rise to recognition), RESILIENCE (interruption followed by recovery), STEADY (consistent scholarship without dramatic arcs), and ASCETIC (research-focused, minimal personal content). Classification is performed via LLM with temperature = 0:

\begin{equation}
\text{NA} = \underset{a \in \{\text{TRIUMPH, RESILIENCE, STEADY, ASCETIC}\}}{\arg\max}\, P(a \mid \text{events})
\end{equation}

accompanied by a three-dimensional affective tone score (positive, negative, neutral) summing to 1.0, extracted simultaneously. Within-model reliability was confirmed by independent re-classification of five scientists with temperature = 0, yielding 100\% agreement on archetype assignment and a mean absolute deviation of 0.04 on affective tone scores. The four-type simplification of Reagan et al.'s six-shape taxonomy is our adaptation; the LLM-based classification methodology is described in Section~\ref{sec:oral}. \textbf{Curatorial action:} faceted browsing by narrative type, collection gap analysis (e.g., all TRIUMPH, no ASCETIC $\rightarrow$ genre bias), researcher filtering by narrative trajectory.

\textbf{MI5: Wikidata Enrichment Ratio (WER).} WER quantifies the unique contribution of a special collection to the global linked data ecosystem by measuring the ratio of LLM-extracted facts to facts already present in Wikidata for the same entities:

\begin{equation}
\text{WER} = \frac{F_{\text{LLM}}}{F_{\text{WD}}}
\end{equation}

where $F_{\text{LLM}}$ is the total number of structured facts extracted by our LLM pipeline from the nianbiao, and $F_{\text{WD}}$ is the total number of comparable facts in Wikidata for the matching scientist Q-items (measured via a 142~GB full Wikidata dump; see Section~\ref{sec:wikidata}). WER is proposed here as a novel metric; while Wikidata coverage analysis exists \cite{allisoncassin2018wikidata}, a quantitative per-collection enrichment ratio has not, to our knowledge, been formalized. Our corpus: mean WER = 36.8$\times$ across 15 scientists with Wikidata entries. The per-field enrichment ratios reveal a structural pattern: education (9.1$\times$), awards (22.5$\times$), and career affiliations (35.3$\times$) show substantial but variable enrichment, while mentorship, collaboration, and biographical events---three relationship types entirely absent from Wikidata ($F_{\text{WD}} = 0$ for each)---represent wholly novel contributions that only the special collection can provide. \textbf{Curatorial action:} high-WER collections prioritized for Wikidata publication; WER quantifies the unique contribution of each collection and supports evidence-based digitization grant applications.
...
This study advances four connected arguments for computational extraction and quality assessment of unstructured biographical text, organized around three research questions.

\textit{First, narrative structure is computationally measurable (RQ1).} The 26 nianbiao, when analyzed as a corpus rather than as individual texts, reveal systematic patterns—a 57.7\% TRIUMPH dominance, a 19.2\% RESILIENCE minority, a 0.0\% ASCETIC absence, and a 0.69 mean positive affect—that are reproducible, comparable, and auditable. Narrative arc classification (NAT, MI4) transforms qualitative interpretation of biographical text into a computable, queryable feature.

\textit{Second, text-derived features can be operationalized as a quantitative indicator system.} The five-metric framework (UD, MBI, CRC, NAT, WER) provides a computational dashboard applicable to any unstructured biographical corpus: which texts contain unverified claims (high UD), which temporal periods are missing (negative MBI), which texts corroborate each other (high CRC), which narrative types are over- or under-represented (NAT distribution), and which texts offer the greatest enrichment to public knowledge graphs (high WER). These metrics are cheap to compute, interpretable, and directly actionable for downstream applications.

\textit{Third, domain-specific extraction complements public knowledge graphs (RQ2).} Wikidata provides authoritative identity anchors for millions of entities but averages only 1.8 facts per scientist in our cohort and contains zero relational statements (mentorship, collaboration, life events). LLM extraction from nianbiao produces 36.8$\times$ more facts, filling precisely the relationship types that general-purpose KGs lack. The two are architecturally complementary: Wikidata identity backbone + domain extraction relational layer = rich, queryable structured biographical data.

\textit{Fourth, the enrichment effect is quantifiable and supports iterative re-assessment (RQ3).} Publishing our 26-scientist extraction to Wikidata would contribute approximately 1,717 net-new linked data statements at a cost of approximately \$0.0007 per fact. After enrichment, the MI1--MI5 indicators can be recomputed to assess whether extraction quality improved—for instance, whether CRC rises from 0.0\% as enriched Wikidata links create cross-text connections, or whether UD decreases as public data corroboration resolves uncertain claims. Three categories of content remain irreducibly scarce even after maximum enrichment: personal reflections never formally published, tacit experimental knowledge, and informal mentor networks—findings that guide where further text acquisition, rather than automated enrichment, is needed. This closed-loop framework—assess $\rightarrow$ enrich $\rightarrow$ re-assess $\rightarrow$ follow up—transforms text extraction quality assessment from a one-time step into an iterative, computationally-driven process.

The practical contribution is a shift from passive text archiving to proactive, computationally-driven extraction and quality assessment. The pathway—LLM extraction → indicator dashboard (MI1--MI5) → Wikidata-aligned identity resolution → linked data publication—is technically demonstrated, quantitatively validated at 97.5\% precision, and economically feasible at scale. The methodology is replicable across languages and text types, offering a general framework for computational extraction of unstructured biographical text that bridges domain-specific corpora and the global knowledge graph ecosystem.
