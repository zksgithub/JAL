...
with overall significance assessed via $\chi^2 = \sum_d z_d^2$, $df = D - 1$. The $\chi^2$ framework is standard inferential statistics; the reminiscence bump concept originates in autobiographical memory research \cite{rubin1986autobiographical}. Our contribution is the operationalization of per-decade $z_d$ as a named, actionable curation metric (MBI) that maps statistical significance to specific archival interventions. Our corpus: $\chi^2 = 842.2$, $df = 12$, $p < 10^{-172}$, with 1960s--1970s under-representation ($z < -2.7$) coincident with the Cultural Revolution. \textbf{Curatorial action:} flag temporal gaps in metadata; prioritize acquisition of contemporaneous institutional records to fill documented silences.
...
where $C_{\text{total}}$ is the total number of verifiable fact claims (career organizations, educational institutions, named collaborators) and $C_{\text{confirmed}}$ is the subset confirmed by $\geq 1$ other collection. CRC is conceptually inspired by the Social Networks and Archival Context (SNAC) project's vision of interconnected archival description \cite{larson2015connecting}, adapted here into a quantitative collection-level metric. Our corpus: CRC = 0.0\% across 26 chronologies---complete narrative isolation, confirming that these chronologies were compiled independently without cross-referencing. \textbf{Curatorial action:} low-CRC collections identified for cross-referencing projects; high-CRC clusters indicate well-triangulated factual cores suitable as linked data anchors.
