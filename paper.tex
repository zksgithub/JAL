...
\title{Uncertainty, Silence, and Isolation:\\
       Computational Analysis of Oral History Features\\
       in Chinese Scientist Chronologies\\
       for Library Special Collections}

\begin{abstract}
Academic libraries serve as custodians of scientific heritage through special collections, yet scientist archives remain fragmented by institution and organized around individual donors rather than intellectual networks. This study applies large language model (LLM)-based semantic extraction and multi-relational knowledge graph analysis to 26 chronological biographies (nianbiao) of Chinese senior scientists (born 1898--1935), comprising 2,582 discrete life events. We develop a three-tier hybrid extraction architecture combining deterministic regex with LLM deep extraction across nine entity categories, achieving a 12.7$\times$ improvement in entity coverage and 4$\times$ more relationship types compared to traditional jieba-based named entity recognition. The extraction pipeline reveals institutional networks centered on Tsinghua (42.3\%), Peking (30.8\%), and Jiaotong (30.8\%) universities, plus previously invisible mentorship chains (e.g., Xin Yixin$\rightarrow$Huang Xuhua at Jiaotong University) and professional collaboration networks spanning national defense projects. A three-generation cohort analysis reveals no significant difference in institutional mobility (Mann-Whitney $U$, $p = 0.807$). We argue that LLM-based extraction transforms the metadata creation bottleneck in library special collections. We propose a minimal metadata schema aligned with CIDOC-CRM and VIAF, and demonstrate that knowledge graph methods can reposition academic libraries from passive collection custodians to active publishers of linked scientific heritage data.
\end{abstract}

\begin{keyword}
academic libraries \sep special collections \sep scientist archives \sep knowledge graph \sep chronologies \sep nianbiao \sep digital curation \sep China
\end{keyword}
...
This study addresses the gap between isolated biographical collections and network-aware special collections through a computational analysis of 26 Chinese senior scientist chronologies. We pose three research questions:

\begin{enumerate}[label=RQ\arabic*:,leftmargin=*]
    \item What institutional and generational patterns emerge from computational extraction of scientist chronologies?
    \item How can knowledge graph analysis inform collection development and discovery strategies for scientist special collections?
    \item What metadata frameworks are needed to support interoperable, linked-data approaches to scientist archives?
\end{enumerate}
...
\textbf{Mentorship Networks.} The LLM extraction identified 74 mentorship relationships across 94 nodes (Figure~\ref{fig:mentorship}). Four scientists in the corpus were identified as mentors to other corpus scientists, creating a direct intellectual genealogy: Feng Duan (冯端, condensed matter physics) mentored Dai Yuanben (戴元本, theoretical physics) at Nanjing University; Shen Keqi (沈克琦, physics education) mentored multiple scientists in the Peking University physics department. Beyond intra-corpus links, several historically significant mentorship chains emerged. Huang Xuhua (黄旭华, nuclear submarine chief designer) was mentored by Xin Yixin (辛一心), Ye Zaifu (叶在馥), and Wang Gongheng (王公衡)---three foundational figures in Chinese naval architecture at Jiaotong University. These relationships contextualize individual scientific achievement within recognizable intellectual lineages, a dimension entirely absent from collection-level archival description.

\textbf{Collaboration Networks.} The collaboration network (Figure~\ref{fig:collaboration}) contains 268 co-worker and peer relationships. The densest collaboration clusters center on scientists in large-scale national projects: Huang Xuhua's nuclear submarine program connected him to Peng Shilu (彭士禄), Zhao Renkai (赵仁恺), and Huang Weilu (黄纬禄); Shen Zhiyun's (沈志云) railway vehicle research linked him to colleagues at Southwest Jiaotong University and the China Academy of Railway Sciences. Several scientists appear as ``bridge nodes'' across institutional boundaries---for example, Wang Dezhi (王德滋, petrology) collaborated with colleagues at both Nanjing University and the Chinese Academy of Geological Sciences, connecting academic and government research sectors.
...
To contextualize the depth of LLM-based extraction, we compared the 26-scientist knowledge graph against Wikidata, the largest open person-centric linked data platform. Wikidata entries for individuals in this cohort typically contain basic demographic properties (birth/death, nationality, occupation) and 1--2 educational affiliations, drawn from Wikipedia infoboxes and manual contributions. Relationship types such as mentors, collaborators, and life events are absent from the Wikidata records of all 26 scientists in our corpus.

Table~\ref{tab:wikidata} and Figure~\ref{fig:wikidata} present the field-level comparison. Across 26 scientists, Wikidata contains a total of 77 structured facts (mean = 3.0 per scientist), while the LLM extraction pipeline produced 1,765 facts (mean = 67.9 per scientist)—a 22.9$\times$ increase. The gap is most pronounced in relationship-oriented fields: Wikidata contains zero mentorship, zero collaboration, and zero biographical event statements, whereas LLM extraction recovered 74 mentors, 268 collaborators, and 523 key life events. Even in fields where Wikidata has coverage, the depth differs markedly: education (5.0$\times$), career affiliations (10.7$\times$), and awards (10.5$\times$).

\begin{table}[ht]
\centering
\caption{Wikidata vs.\ LLM extraction: field-level fact counts across 26 scientists.}
\label{tab:wikidata}
\begin{tabular}{lrrr}
\toprule
Field & Wikidata & LLM Extraction & Ratio \\
\midrule
Education institutions & 42 & 210 & 5.0$\times$ \\
Awards \& honors & 32 & 337 & 10.5$\times$ \\
Career organizations & 33 & 353 & 10.7$\times$ \\
Mentors & 0 & 74 & new \\
Collaborators & 0 & 268 & new \\
Key life events & 0 & 523 & new \\
\midrule
Total & 77 & 1,765 & 22.9$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
The findings of this study converge on four interconnected arguments that, taken together, describe a complete pipeline for transforming isolated oral history collections into computable, interoperable, and globally connected scientific heritage data.
...
\begin{enumerate}[label=\textbf{MI\arabic*:},leftmargin=*]
    ...
    \item \textbf{Wikidata Enrichment Ratio (WER):} the ratio of LLM-extracted facts to Wikidata facts for the same entity. Our mean WER = 22.9$\times$, with mentorship, collaboration, and biographical events entirely absent from Wikidata. \textbf{Curatorial action:} high-WER collections prioritized for Wikidata publication; WER quantifies the unique contribution of each special collection to the global linked data ecosystem.
\end{enumerate}
...
The comparison with Wikidata (Section~\ref{sec:wikidata}) reveals the structural relationship between special collections and public knowledge graphs. Wikidata provides ``mile-wide, inch-deep'' coverage: authoritative identity anchors (birth/death, nationality, occupation, 1--2 education affiliations) for millions of entities, but no relational depth. For our 26 scientists, Wikidata contains 77 total facts---an average of 3.0 per scientist---and zero statements for mentorship, collaboration, or biographical events.

LLM extraction from nianbiao produces ``foot-deep'' coverage: 1,765 facts across nine categories, including 74 mentors, 268 collaborators, and 523 key life events. The 22.9$\times$ enrichment ratio (WER) is not a critique of Wikidata but a quantification of what domain-specific special collections uniquely contribute. Wikidata's strength is breadth and identity resolution; the nianbiao's strength is depth and relational richness. The two are architectural complements.
...
The final argument concerns what happens \textit{after} library-collection facts are published to Wikidata or a similar public knowledge graph—the alignment process and its measurable effects.
...
\subsubsection{Quantifying the Enrichment Effect}

We can project the net enrichment effect of publishing our 26-scientist extraction to Wikidata. Starting from Wikidata's baseline of 77 facts, our LLM pipeline produces 1,765 facts. After deduplication against existing Wikidata statements, the projected net-new contribution is approximately 1,688 statements, distributed as follows:

\begin{itemize}[leftmargin=*]
    \item Education institutions: 168 new statements (42 existing → 210 extracted, 80.0\% net-new)
    \item Awards and honors: 305 new statements (32 existing → 337 extracted, 90.5\% net-new)
    \item Career organizations: 320 new statements (33 existing → 353 extracted, 90.7\% net-new)
...
This study has several limitations. The 26-chronology corpus reflects specific institutional priorities rather than a representative sample of Chinese scientific biography. LLM extraction, while precise, introduces reproducibility concerns mitigated by temperature=0 and structured JSON formatting. The Wikidata comparison uses estimated baseline figures rather than live API queries, due to network constraints during the study period; future work should replicate this comparison against live Wikidata SPARQL endpoints and implement the automated publication pipeline described in Section~\ref{sec:wikidata}. The proposed indicator system (UD, MBI, CRC, NAT, WER) requires validation on larger and more diverse oral history corpora, and the cost-effectiveness analysis assumes DeepSeek-V3 pricing that may not persist.
...
\textit{Third, special collections are complements to—not competitors with—public knowledge graphs.} Wikidata provides authoritative identity anchors for millions of entities but averages only 3.0 facts per scientist in our cohort and contains zero relational statements. LLM extraction from nianbiao produces 22.9$\times$ more facts, filling precisely the relationship types that Wikidata lacks. The two are architecturally complementary: Wikidata identity backbone + nianbiao relational layer = rich, queryable scientific heritage data.

\textit{Fourth, the enrichment effect is quantifiable and economically feasible.} Publishing our 26-scientist extraction to Wikidata would contribute approximately 1,688 net-new linked data statements at a cost of approximately \$0.0007 per fact. The enrichment propagates through the linked data ecosystem via entity completeness (stubs become hubs), cross-collection discoverability (any researcher can find connected archives), and network effects (each new statement links to additional entities). At \$0.05 per scientist, the pipeline is within reach of even modestly resourced special collections programs.
